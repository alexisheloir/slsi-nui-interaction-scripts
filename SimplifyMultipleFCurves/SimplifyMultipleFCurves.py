...
def scanCurvesInfo(fcurves, sframe, eframe):
    ...
    # When a new keyframe is found, the value is taken from all curves. If a curve has not a keyframe for that time, its value is evaluated.
    done = False
    while(not done):
        ...
        if(next_min_time != sys.maxsize):
            ...
            for i, curve in enumerate(fcurves):
                ...
                out_dict[curve_info].append(newKFInfo)
...
class GRAPH_OT_SimplifyMultipleCurves(bpy.types.Operator):
    ...
    normalize = BoolProperty(name="Normalize",
                            description="Perform simplification on normalized curves. It means that before deciding which frames too keep, the curves are internally normalized in a [0,1] range, so that all curves have the same 'importance'. Without normalization, generally the wider curves are 'sucking away' keyframes from smaller ones",
                            default=True)
                            
    
    def draw(self, context):
        self.layout.prop(self, 'normalize')
        self.layout.prop(self, 'simplification', expand=True)
    ...
    # Normalized version, if required
    normalized_fcurves_data = None
    ...
    def execute(self, context):
        ...
        # Will be in fact invoked at first call, when new curves are selected and the operator is created.
        if not self.fcurves_data:
            print("==========================> Storing curves <============================")
            print_curves_info(selected_fcurves)
            self.fcurves_data, self.fcurves_max_keyframes = scanCurvesInfo(fcurves=selected_fcurves, sframe=sframe, eframe=eframe)
            print("Stored KFs=" + str(self.fcurves_max_keyframes))
            
        if((not self.normalized_fcurves_data) and self.normalize):
            self.normalized_fcurves_data = normalizeCurvesInfo(self.fcurves_data)
        ...
        if(self.normalize):
            fcurves_max_offset = get_max_offset(self.normalized_fcurves_data)
        else:
            fcurves_max_offset = get_max_offset(self.fcurves_data)
       
        print("Max Error among curves=" + str(fcurves_max_offset))
        
        err = fcurves_max_offset * self.simplification / 100.0
        print("Simplifying with error: "+str(err))

        if(self.normalize):
            kept_indices = simplify_curves(curves_data=self.normalized_fcurves_data, n_frames=self.fcurves_max_keyframes, error=err)
        else:
