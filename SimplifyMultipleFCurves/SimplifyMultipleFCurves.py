#     "Simplify Multiple F-Curves" is a Blender addon to simplify the keyframes of multiple F-Curves at once
#     retaining an alignment of the keyframes between curves.
...
class KFInfo:
    """This class is used to keep all the values needed to reconstruct a keyframe: coordinates,
     and eventually the coordinates of the tangent handles.
    """
...
#
#
def altitude(point1, point2, pointn) -> float:
...
def get_max_offset(curves_data) -> float:
...
def simplify_curves(curves_data, n_frames, error):
    ...
    # Recurse to retrieve the indices of the keyframes to keep
...
class GRAPH_OT_SimplifyMultipleCurvesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        self.layout.label("Simplify by Max Keyframes:")
        self.layout.prop(context.scene, "simplify_fcurves_max_keyframes")
        self.layout.operator("graph.simplify_multiple_curves_kf", text='Simplify')
        self.layout.separator()
        self.layout.label("Simplify by Error:")
        self.layout.prop(context.scene, "simplify_fcurves_error")
        self.layout.operator("graph.simplify_multiple_curves", text="Simplify")
        self.layout.separator()
...
def register():
    ...
    bpy.types.Scene.simplify_fcurves_error =\
        bpy.props.FloatProperty(name="Max Error (%)",
                                description="Error Percentage. With 0 the curve won't be simplified."
                                            " With 100% the curve will be fully simplified"
                                            " (only border keyframes are retained).",
                                min=0.0,
                                max=100.0,
                                subtype="PERCENTAGE",
